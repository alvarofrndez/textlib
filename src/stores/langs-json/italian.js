export default {
    route_information: 'informazione',
    route_otra: 'un altro',
    choose_file_name: 'Scegli il nome del file',
    accept: 'Accetta',
    text_to_compare1: 'Testo da comparare 1',
    text_to_compare2: 'Testo da comparare 2',
    text_to_convert: 'Testo da convertire',
    reset: 'Ripristina',
    execute: 'Esegui',
    result: 'Risultato',
    placeholder_input: 'Inserisci il testo...',
    text_to_replace: 'Testo da sostituire',
    replace_text: 'Sostituisci testo',
    dropbox_options:{
        compare: 'comparare',
        lowercase: 'a minuscole',
        uppercase: 'a maiuscole',
        camelcase: 'a camelcase',
        capitalletter: 'a lettera maiuscola',
        encode: 'codificare',
        decode: 'decodificare',
        replace: 'sostituire'
    },
    langs_options:{
        spanish: 'Spagnolo',
        english: 'Inglese',
        italian: 'Italiano',
        portuguese: 'Portoghese',
        german: 'Tedesco'
    },
    title_options:{
        import_file: 'Importa il file',
        download_file: 'Scarica il file',
        clear_inputs: 'Cancella i dati',
        copy: 'Copia'
    },
    compare_text_results:{
        equals: 'Testi uguali',
        distincts: 'Testi distinti'
    },
    toast_options:{
        file_not_found: 'File non trovato',
        copy_text: 'Testo copiato',
        copy_text_error: 'Errore durante la copia del testo',
        empty_text: 'Il testo è vuoto',
        error_replace_text: 'Si è verificato un errore con i dati'
    }
}
